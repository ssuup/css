...
'image': {//图片
	...
	'qrcode': function(){//生成二维码
		...
		$(".glyphicon-phone").on('show.bs.popover', function () {
			$(".glyphicon-phone").attr('data-content','<img class="ff-qrcode" src="https://cdn.feifeicms.co/qrcode/1.0/?w=150&h=150&url='+encodeURIComponent(feifei.browser.url)+'"/>');
		})
	},
	'vcode':function(){//安全码
...
'search': {//搜索
	...
	'autocomplete': function(){
		...
		$.getScript("https://cdn.bootcss.com/jquery.devbridge-autocomplete/1.2.26/jquery.autocomplete.min.js", function(response, status) {
...
'score': {//评分
	'raty': function(){
		if( $('.ff-score').length ){
			...
			$("<link>").attr({ rel: "stylesheet",type: "text/css",href: "https://cdn.bootcss.com/raty/2.7.1/jquery.raty.min.css"}).appendTo("head");
			//
			$.getScript("https://cdn.bootcss.com/raty/2.7.1/jquery.raty.min.js", function(response, status) {
...
'playurl': {//播放地址
	'download': function(){
		if($(".ff-playurl-down").length){
			$.getScript("https://cdn.feifeicms.co/download/xunlei.js",function(){
				//3.4增加鼠标点击去掉文件名标题
	...
	'tongji': function(){
		if($("#cms_player").length){
			$.getScript("https://cdn.feifeicms.co/tongji/3.5/");
		}
	},
	'active':function(){
